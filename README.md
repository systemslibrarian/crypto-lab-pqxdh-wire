...
4. **Reuse the fallback** — exhaust one-time PQ prekeys and observe two sessions identify the same last-resort KEM public key without falsely claiming reuse alone reveals either session key.
...
6. **Follow the ratchet** — run a reduced three-step X25519 root-key model that heals classically but remains readable under the curve-break model.
...
The Playwright gate builds before serving, drives every real UI state at desktop and 380 px, runs Axe WCAG 2.1 A/AA, asserts Axe's incomplete bucket, measures text and non-text contrast, checks reflow and keyboard-reachable scrollers, independently recomputes HKDF with Node's `crypto`, and exercises both negative claims. CI blocks deployment unless all checks pass.
